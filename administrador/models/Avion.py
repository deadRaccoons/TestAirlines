...
class Avion(object):
    ...
    def disponibles(fecha, hora, lugar):
        con = Conexion()
        disponibles = []
        todos = con.consultar("select viaje.idavion from avion join viaje on avion.idavion = viaje.idavion where fechallegada = '"+ fecha +"' and (select cast(horallegada as time) +'02:00:00') <= '"+ hora +"' and disponible = 'y' and destino = '"+ lugar +"' group by viaje.idavion having count(*) < 2"):
